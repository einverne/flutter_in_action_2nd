...
在 Flutter 中，Align 是一个基础的布局组件，它允许开发者将其子组件（child）在其自身范围内进行精确的对齐和定位。Align 主要有两个核心功能：一是根据指定的对齐方式定位其子组件，二是可以根据子组件的尺寸来调整自身的大小。
...
- `alignment` : 这是 Align 最重要的属性，用于指定子组件的对齐方式。接收一个`AlignmentGeometry`类型的值，表示子组件在父组件中的起始位置。`AlignmentGeometry` 是一个抽象类，它有两个常用的子类：`Alignment`和 `FractionalOffset`，我们将在下面的示例中详细介绍。
- `child`, 需要被对齐的子组件。
- `widthFactor`和`heightFactor`是用于确定`Align` 组件本身宽高的属性；它们是两个缩放因子，会分别乘以子元素的宽、高，最终的结果就是`Align` 组件的宽高。如果值为`null`，则组件的宽高将会占用尽可能多的空间。

Align 的定位能力很大程度上依赖于 Alignment 类。Alignment 使用一个二维坐标系来描述对齐点：

- 坐标系的原点 (0.0, 0.0) 位于矩形的中心。
- 左上角是 (-1.0, -1.0)。
- 右下角是 (1.0, 1.0)。
